...
# Файлы конфигурации и локализации приложения
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"lang": "ru", "db_path": "diary_tasks.ics"}
...
def load_data():
    """Считывает задачи из универсального файла iCalendar (.ics)"""
    config = load_config()
    ...
    try:
            ...
            for component in cal.walk():
                if component.name == "VEVENT":
                    ...
                    if categories:
                        ...
                        cat_list = [
                            str(c.to_ical().decode("utf-8")) for c in categories
                        ]
                        tags_str = " " + " ".join(
        ...
        messagebox.showerror(
            "Error Reading ICS", f"Не удалось прочитать файл календаря:\n{str(e)}"
        )
...
def save_data(tasks_data):
    """Сохраняет задачи в универсальный международный формат iCalendar (.ics)"""
    config = load_config()
    db_path = config.get("db_path", "diary_tasks.ics")
    ...
    for date_str, tasks_list in tasks_data.items():
        ...
        for task in tasks_list:
            ...
            if tags:
                event.add("categories", [t.strip() for t in tags if t.strip()])
    ...
    try:
        ...
        messagebox.showerror(
            "Error Saving ICS", f"Не удалось сохранить календарь:\n{str(e)}"
        )
...
class DiaryApp:
    def __init__(self, root):
        ...
        self.start_of_week = self.current_date - datetime.timedelta(
        ...
        tk.Label(
            top_settings_panel, text="Language:", bg="#eceff1", font=("Arial", 8)
        ).pack(side=tk.LEFT, padx=(10, 2))

        self.lang_combo = ttk.Combobox(
            top_settings_panel, values=["ru", "en", "de", "ua"], width=5, state="readonly"
        )
        ...
        self.db_path_label = tk.Label(
            top_settings_panel,
            text=os.path.basename(self.config.get("db_path")),
            bg="#eceff1",
            fg="gray",
            font=("Arial", 8, "italic"),
        )
    ...
    def create_task_element(self, parent, date_str, task_text):
        ...
        # ИСПРАВЛЕНО: Если эта задача выбрана в поиске, красим её в яркий оранжевый цвет, иначе в стандартный голубой
        ...
        # Функция для изменения цвета при обычном клике мышкой
        def on_single_click(event):
            # Сбрасываем старую подсветку у всех элементов
            self.highlighted_task_key = task_key
            self.display_week()

        # Привязываем одиночный клик к тексту и к самой плашке
        lbl.bind("<Button-1>", on_single_click)
        task_frame.bind("<Button-1>", on_single_click)
        
        # Двойной клик по-прежнему открывает окно редактирования
    ...
    def do_search(self):
        ...
        def jump_to_date(d_str, full_text):
            ...
            # ИСПРАВЛЕНО: Запоминаем ключ выбранной задачи для подсветки перед перерисовкой экрана
            self.highlighted_task_key = f"{d_str}_{full_text}"
            
            self.display_week()
            self.notebook.select(0) # Переводим пользователя на главную вкладку
        ...
        for date_str in sorted_dates:
            for task in self.data[date_str]:
                if not query or query in task.lower():
                    ...
                    # ИСПРАВЛЕНО: Передаем в команду клика не только дату, но и сам текст задачи
                    btn = tk.Button(
    ...
    def show_activity_chart(self):
        ...
        tk.Button(
            mode_frame,
            text=(
                self.get_txt("mode_tags")
                if self.chart_mode == "tags"
                else self.get_txt("mode_tasks")
            ),
        ...
        tk.Label(
            left_panel, text=self.get_txt("filter_streams"), font=("Arial", 9, "bold")
        ).pack(anchor="w", padx=5, pady=5)
        list_scroll_frame = tk.Frame(left_panel)
        list_scroll_frame.pack(fill=tk.BOTH, expand=True)

        self.list_text_container = tk.Text(
            list_scroll_frame, wrap=tk.NONE, bd=0, bg="#fafafa", highlightthickness=0
        )
        list_scrollbar = tk.Scrollbar(
            list_scroll_frame, orient="vertical", command=self.list_text_container.yview
        )
        ...
        if not items_list:
            ...
            self.list_text_container.insert(
                tk.END, f" {self.get_txt('no_data')} {months[month-1]}."
            )
            self.list_text_container.configure(state=tk.DISABLED)
            tk.Label(
                main_content,
                text=f"{self.get_txt('no_data')} {months[month-1]} {year}.",
                font=("Arial", 12),
            ).pack(pady=50)
            return
        ...
        for day in range(1, num_days + 1):
            if datetime.date(year, month, day).weekday() in [5,6]:
                ax.axvspan(
                    day - 0.5, day + 0.5, facecolor="#96d2ee", alpha=0.15, zorder=0
                )
                ...
                if self.chart_mode == "tags":
                    for t_name in items_list:
                        ...
                        if matching:
                            ...
                            tooltip = (
                                f"{day} {self.get_txt('tooltip_day')} | {self.get_txt('tooltip_tag')} [#{t_name}]:\n• "
                                + "\n• ".join(matching)
                            )
                            bar, = ax.plot(
                                [day - 0.45, day + 0.45],
                    ...
                    for t_name in items_list:
                        ...
                        if has_task:
                            ...
                            lbl_tags = (
                                " (" + ", ".join(tags_found) + ")"
                                if tags_found
                                else f" ({self.get_txt('tooltip_no_labels')})"
                            )
        ...
        for item_name in items_list:
            ...
            tk.Frame(
                item_frame, width=12, height=12, bg=f"#{rgb[0]:02x}{rgb[1]:02x}{rgb[2]:02x}"
            ).pack(side=tk.LEFT, padx=4)
            cb_var = tk.BooleanVar(value=True)
            tk.Checkbutton(
                item_frame,
                text=f"#{item_name}" if self.chart_mode == "tags" else item_name,
                variable=cb_var,
                font=("Arial", 9),
                bg="#fafafa",
                activebackground="#fafafa",
