...
def load_locale():
    if os.path.exists(LOCALE_FILE):
        with open(LOCALE_FILE, "r", encoding="utf-8") as f: return json.load(f)
    return {}

class DiaryApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Completed Task")
        self.root.iconbitmap('icon.ico')
        self.root.geometry("1200x780")
        
        # 1. ОБЪЯВЛЯЕМ ВСЕ ПЕРЕМЕННЫЕ СОСТОЯНИЙ И ДАТ (ИСПРАВЛЕНО)
        ...
        # 2. ЖЕСТКАЯ БЛОКИРОВКА ГЕОМЕТРИИ ОКНА
        self.root.pack_propagate(False)
        self.root.grid_propagate(False)

        self.root.protocol("WM_DELETE_WINDOW", self.on_close_app)

        # 3. ЗАГРУЗКА БАЗЫ ДАННЫХ И ЛОКАЛИЗАЦИИ
        self.data = load_data()
        self.locales = load_locale()
        
        if "__config__" in self.data and "lang" in self.data["__config__"]:
            self.lang = self.data["__config__"]["lang"]
        else:
            self.lang = "ru" if "ru" in self.locales else list(self.locales.keys())

        # Селектор языка
        lang_panel = tk.Frame(self.root, bg="#eceff1", pady=2)
        lang_panel.pack(fill=tk.X)
        tk.Label(lang_panel, text="Language:", bg="#eceff1", font=("Arial", 8)).pack(side=tk.LEFT, padx=10)
        
        self.lang_combo = ttk.Combobox(lang_panel, values=["ru", "en", "de", "uk"], width=5, state="readonly")
    ...
    def get_txt(self, key):
        # Безопасное чтение перевода по ключу
    ...
    def change_language(self, event):
        ...
        # ИСПРАВЛЕНО: Создаем или обновляем служебный блок __config__ в файле данных
        if "__config__" not in self.data:
            self.data["__config__"] = {}
        self.data["__config__"]["lang"] = self.lang
        save_data(self.data) # Сохраняем изменения в diary_data.json

        # Переименовываем вкладки на лету
        ...
        # Полностью перерисовываем элементы окон под новый язык
        self.create_widgets()
        self.display_week()
        self.do_search()

    def create_widgets(self):
        # Очищаем старую панель навигации, если она была создана
        ...
        tk.Button(self.nav_frame_container, text=self.get_txt("btn_today"), command=self.go_to_today, bg="#e0f7fa").pack(side=tk.LEFT, padx=5)
        tk.Button(self.nav_frame_container, text=self.get_txt("btn_go_date"), command=self.go_to_specific_date).pack(side=tk.LEFT, padx=5)

        week_nav_container = tk.Frame(self.nav_frame_container, bg="#f5f5f5")
        week_nav_container.pack(side=tk.LEFT, expand=True)

        tk.Button(week_nav_container, text=self.get_txt("btn_prev_week"), command=self.prev_week).pack(side=tk.LEFT, padx=2)
        self.week_label = tk.Label(week_nav_container, text="", font=("Arial", 12, "bold"), bg="#f5f5f5")
        self.week_label.pack(side=tk.LEFT, padx=15)
        tk.Button(week_nav_container, text=self.get_txt("btn_next_week"), command=self.next_week).pack(side=tk.LEFT, padx=2)

        self.weeks_frame = tk.Frame(self.tab_main)
        self.weeks_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        # Запрещаем фрейму менять свои размеры в зависимости от наличия/отсутствия задач
        self.weeks_frame.grid_propagate(False)
        self.weeks_frame.pack_propagate(False)

        # Строка столбцов намертво растягивается по вертикали вслед за окном
        self.weeks_frame.rowconfigure(0, weight=1)

        # Гарантируем, что все 7 колонок всегда имеют абсолютно одинаковую ширину (uniform)
        for i in range(7):
        ...
        tk.Label(self.search_top_frame, text=self.get_txt("search_label"), font=("Arial", 10, "bold"), bg="#f5f5f5").pack(side=tk.LEFT, padx=5)
        ...
        # Текст кнопки сортировки зависит от направления
        sort_text = self.get_txt("btn_sort_new") if self.search_descending else self.get_txt("btn_sort_old")
        self.btn_sort = tk.Button(self.search_top_frame, text=sort_text, command=self.toggle_search_sort, bg="#fff3e0", font=("Arial", 9, "bold"), padx=10)
        self.btn_sort.pack(side=tk.LEFT, padx=10)

        tk.Button(self.search_top_frame, text=self.get_txt("btn_export"), command=self.export_to_excel, bg="#e8f5e9", font=("Arial", 9, "bold"), padx=10).pack(side=tk.RIGHT, padx=5)

        self.search_scroll_frame = tk.Frame(self.tab_search)
        self.search_scroll_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

        self.search_text_container = tk.Text(self.search_scroll_frame, wrap=tk.NONE, bd=1, relief=tk.SUNKEN, bg="#fafafa")
        search_scrollbar = tk.Scrollbar(self.search_scroll_frame, orient="vertical", command=self.search_text_container.yview)
    ...
    def display_week(self):
        ...
        self.week_label.config(text=f"{self.start_of_week.strftime('%d.%m.%Y')} — {end_of_week.strftime('%d.%m.%Y')}")
        
        days_ru = self.locales.get(self.lang, {}).get("days", ["Pn", "Vt", "Sr", "Ct", "Pt", "Sb", "Vs"])
        ...
        for i in range(7):
            ...
            tk.Label(day_col, text=f"{days_ru[i]}\n{day_date.strftime('%d.%m')}", font=("Arial", 10, "bold"), bg=hdr_color).pack(fill=tk.X, pady=2)
            tk.Button(day_col, text=self.get_txt("btn_add_task"), command=lambda d=date_str: self.open_add_dialog(d), bg="#e8f5e9", font=("Arial", 9, "bold")).pack(fill=tk.X, padx=4, pady=4)

            tasks_container = tk.Frame(day_col, bg=bg_color)
            tasks_container.pack(fill=tk.BOTH, expand=True, padx=0, pady=0)

            # ИСПРАВЛЕНО: Увеличили запас до 42 пикселей для создания гарантированной "подушки безопасности" текста
            def adjust_text_wrap(event, container=tasks_container):
                for child in container.winfo_children():
                    if isinstance(child, tk.Frame):
                        for sub_child in child.winfo_children():
                            if isinstance(sub_child, tk.Label) and sub_child != child.winfo_children():
    ...
    def create_task_element(self, parent, date_str, task_text):
        task_frame = tk.Frame(parent, bg="#e0f2f1", bd=1, relief=tk.RAISED, pady=4)
        task_frame.pack(fill=tk.X, padx=1, pady=2)

        btn_del = tk.Button(
            task_frame, text="❌", bd=0, bg="#e0f2f1", fg="red", 
        ...
        # ИСПРАВЛЕНО: Явно добавили padx=0, pady=0 для текстовой метки lbl,
        # чтобы графический движок не смещал крайние правые буквы к крестику
        lbl = tk.Label(task_frame, text=task_text, bg="#e0f2f1", anchor="w", 
    ...
    def delete_task(self, date_str, task_text):
        if date_str in self.data and task_text in self.data[date_str]:
            ...
            if not self.data[date_str]: del self.data[date_str]
            save_data(self.data)
            self.display_week()
    ...
    def go_to_today(self):
        self.start_of_week = datetime.date.today() - datetime.timedelta(days=datetime.date.today().weekday())
        self.display_week()

    def go_to_specific_date(self):
        date_str = simpledialog.askstring(self.get_txt("btn_go_date"), f"{self.get_txt('btn_go_date')} (DD.MM.YYYY):")
        if date_str:
            try:
                ...
                messagebox.showerror(self.get_txt("msg_error"), self.get_txt("err_date_format"))
    ...
    def get_all_unique_history(self):
        ...
        for day_tasks in self.data.values():
            for t in day_tasks:
                ...
                if clean_text: tasks.add(clean_text)
        return list(tasks)
    ...
    def open_add_dialog(self, date_str):
        ...
        def refresh_checkboxes():
            ...
            if tags_entry.get() and tags_entry.get()[-1] in [" ", ","]: last_word = ""
            ...
            for tag in visible_tags:
                if tag not in checkbox_vars: checkbox_vars[tag] = tk.BooleanVar(value=False)
                def on_cb_click(t=tag, v=checkbox_vars[tag]):
                    current_text = tags_entry.get().replace(",", " ").split()
                    if current_text and not tags_entry.get()[-1] in [" ", ","]: current_text.pop()
                    if v.get():
                ...
                cb = tk.Checkbutton(tags_text_panel, text=f"#{tag}", variable=checkbox_vars[tag], bg="#fafafa", activebackground="#fafafa", font=("Arial", 9), command=on_cb_click)
        ...
        for tag in unique_tags_list: checkbox_vars[tag] = tk.BooleanVar(value=False)
        refresh_checkboxes()
        ...
        def save_entry():
            ...
            if tsk:
                ...
                if date_str not in self.data: self.data[date_str] = []
                if full_text not in self.data[date_str]: self.data[date_str].append(full_text)
                save_data(self.data)
                self.display_week()
                dialog.destroy()
            else: messagebox.showwarning(self.get_txt("msg_warning"), self.get_txt("warn_empty_task"))
    ...
    def open_edit_dialog(self, date_str, old_task_text):
        ...
        checkbox_vars = {}
        for tag in unique_tags_list: checkbox_vars[tag] = tk.BooleanVar(value=tag in current_task_tags)

        def sync_checkboxes_from_text():
            current_input_tags = [t.lower().strip() for t in tags_entry.get().replace(",", " ").split() if t.strip()]
            for tag, var in checkbox_vars.items(): var.set(tag in current_input_tags)
        ...
        tk.Button(btn_frame, text=self.get_txt("btn_save_changes"), command=save_changes, bg="#a5d6a7", font=("Arial", 9, "bold"), padx=15, pady=3).pack(pady=10)
        ...
        for date_str in sorted_dates:
            ...
            for task in self.data[date_str]:
                if not query or query in task.lower():
                    ...
                    btn = tk.Button(self.search_text_container, text=f" 📅 {dt.strftime('%d.%m.%Y')}  👉  {task} ", anchor="w", justify=tk.LEFT, bg="#f0f4c3", relief=tk.GROOVE, padx=5, pady=1, font=("Arial", 9), command=lambda d=date_str: jump_to_date(d))
                    self.search_text_container.window_create(tk.END, window=btn)
                    self.search_text_container.insert(tk.END, "\n")
        if not found: self.search_text_container.insert(tk.END, self.get_txt("search_no_results"))
        self.search_text_container.configure(state=tk.DISABLED)
    ...
    def export_to_excel(self):
        ...
        for item in items_to_export:
            ...
            rows.append({
                ...
                self.get_txt("excel_weekday"): days_ru[dt.weekday()], 
        ...
        try:
            ...
            messagebox.showinfo(self.get_txt("msg_success"), f"{self.get_txt('success_export')}\n{os.path.abspath(filename)}")
        except Exception as e: messagebox.showerror(self.get_txt("msg_error"), f"Excel Error:\n{str(e)}")

    def on_tab_changed(self, event):
        idx = self.notebook.index(self.notebook.select())
        if idx == 1: self.show_activity_chart()
        elif idx == 2: self.do_search()
    ...
    def show_activity_chart(self):
        for w in self.tab_chart.winfo_children(): w.destroy()
        ...
        months_ru = self.locales.get(self.lang, {}).get("months", ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"])

        tk.Label(mode_frame, text=f"📊 {self.get_txt('chart_title')} {months_ru[month-1]} {year}", font=("Arial", 11, "bold"), bg="#f5f5f5").pack(side=tk.LEFT)
        tk.Button(mode_frame, text=self.get_txt("mode_tags") if self.chart_mode == "tags" else self.get_txt("mode_tasks"), command=self.toggle_chart_mode, bg="#e1f5fe", font=("Arial", 9, "bold")).pack(side=tk.RIGHT)
        ...
        for day in range(1, num_days + 1):
            ...
            if date_str in self.data and date_str != "__config__":
                for t in self.data[date_str]:
                    if self.chart_mode == "tags":
                        for tag in re.findall(r'\[#([^\]]+)\]', t): month_items.add(tag.lower().strip())
                    else:
                        clean_task = re.sub(r'\[#[^\]]+\]', '', t).strip()
                        if clean_task: month_items.add(clean_task)

        items_list = sorted(list(month_items))
        if not items_list:
            self.list_text_container.configure(state=tk.NORMAL)
            self.list_text_container.insert(tk.END, f" {self.get_txt('no_data')} {months_ru[month-1]}.")
            self.list_text_container.configure(state=tk.DISABLED)
            tk.Label(main_content, text=f"{self.get_txt('no_data')} {months_ru[month-1]} {year}.", font=("Arial", 12)).pack(pady=50)
            return
        ...
        for day in range(1, num_days + 1):
            if datetime.date(year, month, day).weekday() in [5, 6]:
                ax.axvspan(day - 0.5, day + 0.5, facecolor='#e0e0e0', alpha=0.15, zorder=0)
            ...
            if date_str in self.data:
                if self.chart_mode == "tags":
                    for t_name in items_list:
                        matching = [re.sub(r'\[#[^\]]+\]', '', t).strip() for t in self.data[date_str] if f"[#{t_name}]" in t.lower()]
                        if matching:
                            idx = items_list.index(t_name)
                            tooltip = f"{day} {self.get_txt('tooltip_day')} | {self.get_txt('tooltip_tag')} [#{t_name}]:\n• " + "\n• ".join(matching)
                            bar, = ax.plot([day - 0.45, day + 0.45], [idx * 0.15, idx * 0.15], color=colors_map[t_name], linewidth=7, solid_capstyle='round', gid=tooltip)
        ...
        def toggle_line_visibility(name, cb_var):
            for line_obj in self.chart_bars[name]: line_obj.set_visible(cb_var.get())
            fig.canvas.draw()
        ...
        for item_name in items_list:
            ...
            tk.Checkbutton(item_frame, text=f"#{item_name}" if self.chart_mode == "tags" else item_name, variable=cb_var, font=("Arial", 9), bg="#fafafa", activebackground="#fafafa", command=lambda n=item_name, v=cb_var: toggle_line_visibility(n, v)).pack(side=tk.LEFT, anchor="w")
            self.list_text_container.window_create(tk.END, window=item_frame)
            self.list_text_container.insert(tk.END, "\n")
        self.list_text_container.configure(state=tk.DISABLED)

        annot = ax.annotate("", xy=(0, 0), xytext=(10, 10), textcoords="offset points", color="white", bbox=dict(boxstyle="round,pad=0.5", fc="black", ec="black", alpha=0.8))
